...
describe('DeepClaw OpenAI image plugin request shape', () => {
  it('does not force deprecated OpenAI Images response_format', async () => {
    const pluginSource = await readFile(
      join(repoRoot, 'resources/openclaw-plugins/deepclaw-openai-image/index.mjs'),
      'utf8',
    );
  ...
  it('omits response_format from generated OpenAI-compatible requests', async () => {
    ...
    try {
      const plugin = await import('../../resources/openclaw-plugins/deepclaw-openai-image/index.mjs');
      ...
      const result = await provider?.generateImage({
        provider: 'deepclaw-openai-image',
        model: 'gpt-image-2',
        prompt: 'paint a fox',
        quality: 'high',
        outputFormat: 'png',
        ...
        agentDir: '/tmp/deepclaw-openai-image-test-agent',
