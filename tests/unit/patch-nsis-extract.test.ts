...
describe('patch-nsis-extract', () => {
  ...
  it('replaces CopyFiles-based extractUsing7za with direct 7z extraction', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'deepclaw-patch-nsis-'));
    ...
    expect(result).toContain('DeepClaw-patched-v2');
    ...
    expect(result).toContain('Restoring previous DeepClaw installation after failed update');
    expect(result).not.toContain('continuing overwrite install anyway');
    expect(patchNsisExtractTemplate(target)).toBe(true);
  });

  it('upgrades stale DeepClaw extract patches that used to continue after extract failure', () => {
    tempDir = mkdtempSync(join(tmpdir(), 'deepclaw-patch-nsis-'));
    const target = join(tempDir, 'extractAppPackage.nsh');
    writeFileSync(
      target,
  ...
  ; DeepClaw-patched: extract directly to $INSTDIR.
  ClearErrors
  Nsis7z::Extract "\${FILE}"
...
!macroend`,
    ...
    const result = readFileSync(target, 'utf8');
    expect(result).toContain('DeepClaw-patched-v2');
    expect(result).toContain('Failed to extract DeepClaw files after multiple attempts.');
    expect(result).toContain('$(decompressionFailed)');
    expect(result).toContain('Quit');
    expect(result).toContain('SetErrorLevel 2');
    expect(result).toContain('Restoring previous DeepClaw installation after failed update');
