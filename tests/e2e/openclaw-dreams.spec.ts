...
test.describe('OpenClaw Dreams', () => {
          ...
          shortTermEntries: [
            {
              ...
              snippet: 'User expects Dreams to be a native DeepClaw interface, not only an external jump.',
              startLine: 4,
              totalSignalCount: 2,
            },
          ],
  ...
  test('renders the native Dreams page and runs a maintenance action', async ({ electronApp, page }) => {
    ...
    await expect(page.getByText('User expects Dreams to be a native DeepClaw interface')).toBeVisible();
    ...
    await installIpcMocks(electronApp, {
      ...
      gatewayRpc: {
        ...
        [stableStringify(['config.patch', {
          ...
          baseHash: configHash,
          note: 'Enable memory dreaming from DeepClaw Dreams.',
        }])]: {
          success: true,
          result: { ok: true },
        },
