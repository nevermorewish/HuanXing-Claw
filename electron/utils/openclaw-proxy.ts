...
interface SyncProxyOptions {
   ...
   * currently disabled in DeepClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync DeepClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because DeepClaw proxy is disabled and preserve mode is enabled');
      return;
    }
