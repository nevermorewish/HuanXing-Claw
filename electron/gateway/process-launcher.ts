...
(function () {
  ...
  if (globalThis.__deepclawFetchPatched) return;
  globalThis.__deepclawFetchPatched = true;

  globalThis.fetch = function deepclawFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      flat['X-OpenRouter-Title'] = 'DeepClaw';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__deepclawPatched) {
        cp.__deepclawPatched = true;
...
export async function launchGatewayProcess(options: {
  ...
  // which causes:
  //   - cross-machine name collisions when multiple OpenClaw/DeepClaw peers
  //     share a LAN (each falls back to "<name> (OpenClaw) (2)")
  //   - self-collisions on multi-homed hosts (Wi-Fi + Tailscale + utun ...)
  //   - "ghost" record collisions after an unclean DeepClaw exit, because
  //     SIGKILL prevents ciao from emitting the mDNS goodbye record.
  //
  // DeepClaw has no UI for LAN gateway discovery today, so the advertiser is
