...
<a href="#почему-deepclaw">Почему DeepClaw</a> •
...
**DeepClaw** — это мост между мощными AI-агентами и повседневными пользователями. Построенный на базе [OpenClaw](https://github.com/OpenClaw), он превращает управление AI через командную строку в доступный и красивый десктоп-опыт — терминал не нужен.

Автоматизация рабочих процессов, управление AI-каналами или планирование интеллектуальных задач — DeepClaw предоставляет интерфейс для эффективного использования AI-агентов.

DeepClaw поставляется с предустановленными лучшими практиками для провайдеров моделей и нативно поддерживает Windows, а также многоязычные настройки. Вы можете тонко настроить расширенные параметры через **Настройки → Дополнительно → Режим разработчика**.
...
Создание AI-агентов не должно требовать владения командной строкой. Философия DeepClaw проста: **мощные технологии заслуживают интерфейса, который уважает ваше время.**

| Проблема | Решение DeepClaw |
|----------|---------------|
...
DeepClaw построен непосредственно на официальном ядре **OpenClaw**. Вместо отдельной установки мы встраиваем среду выполнения в приложение для бесшовного опыта "всё включено".
...
При выборе другого агента через `@agent` DeepClaw переключается непосредственно в контекст этого агента вместо ретрансляции через агента по умолчанию. Рабочие пространства агентов по умолчанию разделены, но более строгая изоляция зависит от настроек песочницы OpenClaw.
...
Для пользовательских идентификаторов учётных записей каналов DeepClaw требует совместимый с OpenClaw канонический формат (`[a-z0-9_-]`, строчные буквы, максимум 64 символа, должен начинаться с буквы или цифры) для предотвращения ошибок маршрутизации.
DeepClaw также включает официальный плагин личного WeChat от Tencent, позволяя подключить WeChat напрямую со страницы Каналы через встроенный QR-код.
...
DeepClaw также предварительно упаковывает полные навыки обработки документов (`pdf`, `xlsx`, `docx`, `pptx`), автоматически развёртывает их в управляемый каталог навыков (по умолчанию `~/.openclaw/skills`) при запуске и включает по умолчанию при первой установке. Дополнительные упакованные навыки (`find-skills`, `self-improving-agent`, `tavily-search`) также включены по умолчанию; если необходимые API-ключи отсутствуют, OpenClaw покажет ошибки конфигурации во время выполнения.
...
Когда совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, DeepClaw автоматически переключается на легковесный зонд `/chat/completions` или `/responses` при проверке API-ключа.

### 🌙 Адаптивные темы
Светлая тема, тёмная тема или синхронизация с системой. DeepClaw автоматически адаптируется к вашим предпочтениям.

### 🚀 Управление автозапуском
В **Настройки → Общие** вы можете включить **Запускать при старте системы**, чтобы DeepClaw автоматически запускался после входа в систему.
...
Скачайте последний релиз для вашей платформы со страницы [Releases](https://github.com/ValueCell-ai/DeepClaw/releases).
...
При первом запуске DeepClaw **Мастер настройки** проведёт вас через:
...
DeepClaw включает встроенные настройки прокси для сред, где Electron, шлюз OpenClaw или каналы вроде Telegram должны выходить в интернет через локальный прокси-клиент.
...
- Если расширенные поля прокси пусты, DeepClaw использует `Прокси-сервер`.
- Сохранение настроек прокси немедленно повторно применяет сеть Electron и автоматически перезапускает шлюз.
- DeepClaw также синхронизирует прокси с конфигурацией канала Telegram в OpenClaw, когда Telegram включён.
- При перезапуске шлюза существующий прокси канала Telegram сохраняется, если прокси DeepClaw отключен.
...
DeepClaw использует **двухпроцессную архитектуру с унифицированным уровнем Host API**. Рендерер обращается к единой абстракции клиента, а Electron Main управляет выбором протокола и жизненным циклом процессов:

```
┌─────────────────────────────────────────────────────────────────┐
│                     Десктоп-приложение DeepClaw                    │
│                                                                 │
│  ┌────────────────────────────────────────────────────────────┐ │
...
- DeepClaw — это приложение Electron, поэтому **один экземпляр приложения обычно отображается как несколько процессов ОС** (main/renderer/zygote/utility). Это нормально.
...
- Нажатие кнопки закрытия окна (`X`) скрывает DeepClaw в трей; это **не** полностью закрывает приложение. Используйте меню трея **Quit DeepClaw** для полного завершения.
...
- использует временный каталог `userData` DeepClaw
...
Мы приветствуем вклад сообщества! Исправления багов, новые функции, улучшения документации или переводы — каждый вклад делает DeepClaw лучше.
...
DeepClaw построен на плечах отличных проектов с открытым исходным кодом:
...
### Партнёрская программа DeepClaw 🚀

Мы запускаем Партнёрскую программу DeepClaw и ищем партнёров, которые могут помочь представить DeepClaw большему числу клиентов, особенно тем, у кого есть потребности в кастомных AI-агентах или автоматизации.

Партнёры помогают связывать нас с потенциальными пользователями и проектами, а команда DeepClaw предоставляет полную техническую поддержку, кастомизацию и интеграцию.
...
DeepClaw выпускается под [лицензией MIT](LICENSE). Вы можете свободно использовать, модифицировать и распространять это программное обеспечение.
