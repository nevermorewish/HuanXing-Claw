...
export function Usage() {
  ...
  useEffect(() => {
    ...
    trackUiEvent('usage.token_usage_fetch_started', {
      generation,
      restartMarker,
    });
    ...
    const safetyTimeout = setTimeout(() => {
      ...
      trackUiEvent('usage.token_usage_fetch_safety_timeout', {
        generation,
        restartMarker,
      });
    ...
    const fetchUsageHistoryWithRetry = async (attempt: number) => {
      trackUiEvent('usage.token_usage_fetch_attempt', {
        generation,
        attempt,
        restartMarker,
      });
      try {
        ...
        setUsagePage(1);
        trackUiEvent('usage.token_usage_fetch_succeeded', {
          generation,
          attempt,
        ...
        if (normalized.length === 0 && attempt < usageFetchMaxAttempts) {
          trackUiEvent('usage.token_usage_fetch_retry_scheduled', {
            generation,
            attempt,
            reason: 'empty',
            restartMarker,
          ...
          if (normalized.length === 0) {
            trackUiEvent('usage.token_usage_fetch_exhausted', {
              generation,
              attempt,
              reason: 'empty',
              restartMarker,
        ...
        if (usageFetchGenerationRef.current !== generation) return;
        trackUiEvent('usage.token_usage_fetch_failed_attempt', {
          generation,
          attempt,
          restartMarker,
  ...
  return (
    <div data-testid="usage-page" className="flex flex-col -m-6 dark:bg-background h-[calc(100vh-2.5rem)] overflow-hidden">
      <div className="w-full max-w-5xl mx-auto flex flex-col h-full p-10 pt-16">

        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-start justify-between mb-12 shrink-0 gap-4">
          <div>
            <h1 data-testid="usage-page-title" className="text-3xl md:text-4xl font-serif text-foreground mb-2 font-normal tracking-tight">
              {t('dashboard:recentTokenHistory.title', '使用情况')}
            </h1>
            <p className="text-sm text-foreground/60 font-medium">
              {t('dashboard:recentTokenHistory.subtitle', '查看 Token 消耗、API 费用和模型使用统计')}
            </p>
          </div>
        </div>
