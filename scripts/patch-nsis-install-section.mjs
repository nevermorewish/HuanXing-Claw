 ...
 * replacement in that inner process, so the old DeepClaw.exe can remain alive,
...
const PATCH_MARKER = 'DeepClaw-patched-v2: run app-running guard in assisted UAC inner instance';
...
const PATCHED_ASSISTED_CHECK = [
  ...
  '  ; UAC inner instance; skipping CHECK_APP_RUNNING there can leave DeepClaw.exe',
