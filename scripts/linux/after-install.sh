...
# Post-installation script for DeepClaw on Linux
...
# Create symbolic link for DeepClaw app binary
if [ -x /opt/DeepClaw/deepclaw ]; then
    ln -sf /opt/DeepClaw/deepclaw /usr/local/bin/deepclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/DeepClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/DeepClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/DeepClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/DeepClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/deepclaw'
...
echo "DeepClaw has been installed successfully."
