...
DeepClaw comes pre-configured with best-practice model providers and natively supports Windows as well as multi-language settings. Of course, you can also fine-tune advanced configurations via **Settings → Advanced → Developer Mode**.
...
Building AI agents shouldn't require mastering the command line. DeepClaw was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**
...
DeepClaw is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
When you target another agent with `@agent`, DeepClaw switches into that agent's own conversation context directly instead of relaying through the default agent. Agent workspaces stay separate by default, and stronger isolation depends on OpenClaw sandbox settings.
...
For custom channel account IDs, DeepClaw enforces OpenClaw-compatible canonical IDs (`[a-z0-9_-]`, lowercase, max 64 chars, must start with a letter/number) to prevent routing mismatches.
DeepClaw now also bundles Tencent's official personal WeChat channel plugin, so you can link WeChat directly from the Channels page with an in-app QR flow.
...
DeepClaw also pre-bundles full document-processing skills (`pdf`, `xlsx`, `docx`, `pptx`), deploys them automatically to the managed skills directory (default `~/.openclaw/skills`) on startup, and enables them by default on first install. Additional bundled skills (`find-skills`, `self-improving-agent`, `tavily-search`) are also enabled by default; if required API keys are missing, OpenClaw will surface configuration errors in runtime.  
...
When a compatible gateway rejects `/models` for non-auth reasons, DeepClaw automatically falls back to a lightweight `/chat/completions` or `/responses` probe during API key validation.

### 🌙 Adaptive Theming
Light mode, dark mode, or system-synchronized themes. DeepClaw adapts to your preferences automatically.

### 🚀 Startup Launch Control
In **Settings → General**, you can enable **Launch at system startup** so DeepClaw starts automatically after login.

### 🔔 Update Prompts
DeepClaw can automatically check for new versions on startup. When an update is available, it shows an in-app prompt; downloading and installing only happen after you choose the action.
...
Download the latest release for your platform from the [Releases](https://github.com/ValueCell-ai/DeepClaw/releases) page.
...
> Note for Moonshot (Kimi): DeepClaw keeps Kimi web search enabled by default.  
> When Moonshot is configured, DeepClaw also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

DeepClaw includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, DeepClaw falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- DeepClaw also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.
- Gateway restarts preserve an existing Telegram channel proxy if DeepClaw proxy is currently disabled.
...
DeepClaw employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:
...
- Clicking the window close button (`X`) hides DeepClaw to tray; it does **not** fully quit the app. Use tray menu **Quit DeepClaw** for complete shutdown.
...
- uses a temporary DeepClaw `userData` directory
...
Partners help connect us with potential users and projects, while the DeepClaw team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/DeepClaw&type=Date" alt="Star History Chart" />
</p>
